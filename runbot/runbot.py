...
class runbot_repo(osv.osv):
    ...
    def git(self, cr, uid, ids, cmd, context=None):
        ...
        for repo in self.browse(cr, uid, ids, context=context):
            ...
            _logger.info("__ git: %s", ' '.join(cmd))
            return subprocess.check_output(cmd)

    def git_export(self, cr, uid, ids, treeish, dest, context=None):
        for repo in self.browse(cr, uid, ids, context=context):
            _logger.debug('__ checkout %s %s %s', repo.name, treeish, dest)
    ...
    def github(self, cr, uid, ids, url, payload=None, ignore_errors=False, context=None):
        ...
        for repo in self.browse(cr, uid, ids, context=context):
            ...
            try:
                ...
                if ignore_errors:
                    _logger.exception('__ Ignored github error %s %r', url, payload)
                else:
                    raise
    ...
    def update_git(self, cr, uid, repo, context=None):
        _logger.debug('__ repo %s updating branches', repo.name)
        ...
        if os.path.isfile(fname_fetch_head):
            ...
            if repo.mode == 'hook' and repo.hook_time and dt2time(repo.hook_time) < fetch_time:
                t0 = time.time()
                _logger.debug('__ repo %s skip hook fetch fetch_time: %ss ago hook_time: %ss ago',
        ...
        for name, sha, date, author, author_email, subject, committer, committer_email in refs:
            ...
            if ref_branches.get(name):
                ...
                _logger.debug('__ repo %s found new branch %s', repo.name, name)
            ...
            if not build_ids:
                _logger.debug('__ repo %s branch %s new build found revno %s', branch.repo_id.name, branch.name, sha)
                build_info = {
                    'branch_id': branch.id,
                    'name': sha,
                    'author': author,
    ...
    def reload_nginx(self, cr, uid, context=None):
        ...
        if ids:
            ...
            try:
                _logger.debug('__ reload nginx')
                pid = int(open(os.path.join(nginx_dir, 'nginx.pid')).read().strip(' \n'))
                os.kill(pid, signal.SIGHUP)
            except Exception:
                _logger.debug('__ start nginx')
                if run(['/usr/sbin/nginx', '-p', nginx_dir, '-c', 'nginx.conf']):
                    ...
                    if not run(['pkill', '-f', '-P1', 'nginx: worker']):
                        _logger.debug('__ failed to start nginx - orphan worker killed, retrying')
                        run(['/usr/sbin/nginx', '-p', nginx_dir, '-c', 'nginx.conf'])
                    else:
                        _logger.debug('__ failed to start nginx - failed to kill orphan worker - oh well')
...
class runbot_build(osv.osv):
    ...
    def find_port(self, cr, uid):
        ...
        # starting port
        icp = self.pool['ir.config_parameter']
        port = int(icp.get_param(cr, uid, 'runbot.starting_port', default=8083))
    ...
    def _get_closest_branch_name(self, cr, uid, ids, target_repo_id, context=None):
        ...
        _logger.debug('__ Search closest of %s (%s) in repos %r', name, repo.name, target_repo_ids)
    ...
    def checkout(self, cr, uid, ids, context=None):
        for build in self.browse(cr, uid, ids, context=context):
            ...
            _logger.debug("__ manual modules_to_test for build %s: %s", build.dest, modules_to_test)
            ...
            if not has_server:
                if build.repo_id.modules_auto == 'repo':
                    ...
                    _logger.debug("__ local modules_to_test for build %s: %s", build.dest, modules_to_test)
                ...
                for extra_repo in build.repo_id.dependency_ids:
                    ...
                    _logger.debug('__ branch %s of %s: %s match branch %s of %s',
            ...
            _logger.debug("__ modules_to_test for build %s: %s", build.dest, modules_to_test)
    ...
    def _local_pg_createdb(self, cr, uid, dbname):
        ...
        _logger.debug("__ __ createdb %s", dbname)
        with local_pgadmin_cursor() as local_cr:
    ...
    def spawn(self, cmd, lock_path, log_path, cpu_limit=None, shell=False, env=None):
        def preexec_fn():
            ...
            lock(lock_path)
        out = open(log_path, "w")
        _logger.debug("__ spawn: %s stdout: %s", ' '.join(cmd), log_path)
    ...
    def github_status(self, cr, uid, ids, context=None):
        ...
        for build in self.browse(cr, uid, ids, context=context):
            ...
            print "______________ github_status status : ", status
            _logger.debug("__ github updating status %s to %s", build.name, state)
    ...
    def job_10_test_base(self, cr, uid, build, lock_path, log_path):
        ...
        # run base test
        print "________________ job_10_test_base lock_path : ", lock_path
        print "________________ log_path : ", log_path
        self._local_pg_createdb(cr, uid, "%s-base" % build.dest)
        print "____________________ %s-base created successfully" % build.dest
        cmd, mods = build.cmd()
        if grep(build.server("tools/config.py"), "test-enable"):
            cmd.append("--test-enable")
        print "____________________ add --test-enable in cmd : "
        pprint(cmd)
        cmd += ['-d', '%s-base' % build.dest, '-i', 'base', '--stop-after-init', '--log-level=test', '--max-cron-threads=0']
        print "____________________ add db filter in cmd : "
        pprint(cmd)
        return self.spawn(cmd, lock_path, log_path, cpu_limit=300)

    def job_20_test_all(self, cr, uid, build, lock_path, log_path):
        build._log('job_20_test_all', 'Start test all modules')
        print "________________ job_20_test_all lock_path : ", lock_path
        print "________________ log_path : ", log_path
        self._local_pg_createdb(cr, uid, "%s-all" % build.dest)
        print "____________________ %s-all created successfully" % build.dest
        cmd, mods = build.cmd()
        if grep(build.server("tools/config.py"), "test-enable"):
            cmd.append("--test-enable")
        ...
        print "_____________________ job_20_test_all : cmd : ", cmd
    ...
    def reap(self, cr, uid, ids):
        while True:
            ...
            if pid == 0:
                break
            _logger.debug('__ reaping: pid: %s status: %s', pid, status)

    def _log(self, cr, uid, ids, func, message, context=None):
        assert len(ids) == 1
        _logger.debug("__ Build %s %s %s", ids[0], func, message)
